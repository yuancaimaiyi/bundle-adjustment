...
//     //check points检查点
//     points3D[26] =cv::Point3d(0,0,0);
//     points3D[27] =cv::Point3d(0,0,0);
//     points3D[28] =cv::Point3d(0,0,0);
//     points3D[29] =cv::Point3d(0,0,0);
//     points3D[30] =cv::Point3d(0,0,0);
//     points3D[31] =cv::Point3d(0,0,0);
     for(int i=0;i<NCAMS;++i)
     {
         points3D[i]=cv::Point3d(0,0,0);
     }
     // 填充像点
    ...
    // 初始化旋转矩阵 (rotation around Z axis)
    R.resize(NCAMS);
    for(int i=0; i<NCAMS; i++) {
    ...
    // 初始化平移 (3 units away in camera Z axis)
    T.resize(NCAMS);
    for(int i=0; i<NCAMS; i++) {
 ...
 /****** 光束法平差 ******/
    ...
    //sba.setParams(cvsba::Sba::MOTIONSTRUCTURE,100,0.0001,0,0,true);
    sba.run(points3D,  pointsImg,  visibility,  cameraMatrix,  R,  T, distCoeffs);
    printf("************内参数矩阵**********\n");
    for(int i=0; i<NCAMS; i++)
    {
        printMatrix(cameraMatrix[i]);
    }
    printf("**********畸变系数**********\n");
    for(int i=0;i<NCAMS;i++)
    {
        printf("第%d个相机畸变\n",i);
        printMatrix(distCoeffs[i]);
    }
    printf("check point x,y,z\n");
    for(int i=0;i<=31;++i)
    {
